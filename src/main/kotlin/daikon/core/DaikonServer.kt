...
abstract class DaikonServer(initializeActions: DaikonServer.() -> Unit = {}) : AutoCloseable {
    ...
    protected val context = ServerContext()
    ...
    fun exception(exception: Class<out Throwable>, action: (Request, Response, Context, Throwable) -> Unit) = exception(exception, ContextExceptionAction(action))
    fun exception(exception: Class<out Throwable>, action: (Request, Response, Throwable) -> Unit) = exception(exception, DummyExceptionAction(action))
    ...
    fun before(path: String = "/*", action: (Request, Response) -> Unit) = before(path, DummyRouteAction(action))
    fun before(path: String = "/*", action: (Request, Response, Context) -> Unit) = before(path, ContextRouteAction(action))
    fun before(path: String = "/*", action: RouteAction): DaikonServer {
        beforeActions.add(Route(Method.ANY, joinPaths(path), action))
        return this
    }

    fun after(path: String = "/*", action: (Request, Response) -> Unit) = after(path, DummyRouteAction(action))
    fun after(path: String = "/*", action: (Request, Response, Context) -> Unit) = after(path, ContextRouteAction(action))
    fun after(path: String = "/*", action: RouteAction): DaikonServer {
        afterActions.add(Route(Method.ANY, joinPaths(path), action))
        return this
    }
